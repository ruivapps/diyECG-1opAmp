...
class Ear(object):
    ...
    def initiate(self):
        ...
        """
        # FIXME: user should be able to pick the right mic. tmp hack
        device = 0
        if sys.argv[-1].isdigit():
            device = int(sys.argv[-1])
        if self.device is None:
            self.device = self.valid_input_devices()[
                device]  # pick the first one
        if self.rate is None:
            self.rate = self.valid_low_rate(self.device)
        if not self.valid_test(self.device, self.rate):
            print("guessing a valid microphone device/rate...")
            # FIXME: user should be able to pick the right mic
            self.device = self.valid_input_devices()[
                device]  # pick the first one
            self.rate = self.valid_low_rate(self.device)
